...
export function buildSearchOptionsFromConfig() {
  const c = getConfig()
  const searchFields = (c.searchFields || c.fields || []).reduce((acc, n) => {
    acc = acc || {}
    acc[n] = {}
    return acc
  }, undefined)

  const resultFields = (c.resultFields || c.fields || []).reduce((acc, n) => {
    acc = acc || {}
    acc[n] = {
      raw: {},
      snippet: {
...
export function buildSortOptionsFromConfig() {
  ...
  return [
    ...
    ...(c.sortFields || []).reduce((acc, sortField) => {
      acc.push({
